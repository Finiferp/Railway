...
import com.example.railway.databinding.FragmentMyNeedsBinding;
...
public class MyNeedsFragment extends Fragment {
    ...
    private FragmentMyNeedsBinding binding;


    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        binding = FragmentMyNeedsBinding.inflate(inflater, container, false);
    ...
    private void apiCall(){
        ...
        String url = "http://192.168.129.37:3000/player/needs";
        //String url = "http://192.168.178.39:3000/player/needs";
                    ...
                    public void onResponse(String response) {
                        try {
                            ...
                            for(int i = 0; i< data.length();i++){
                                ...
                                JSONArray needs = need.getJSONArray("needs");
                                String goods = "";
                                for(int j = 0; j< needs.length(); j++){
                                    JSONObject item = needs.getJSONObject(i);
                                    String goodName = item.getString("good_name");
                                    double consumption = item.getDouble("consumption");
                                    goods += goodName + ": " + consumption + ("\r\n");
                                }
                                MyNeed myNeed = new MyNeed(assetName,assetId,goods);
    ...
    private void populate() {
        ...
        TableLayout tableLayout = binding.myNeedsTableLayout;
