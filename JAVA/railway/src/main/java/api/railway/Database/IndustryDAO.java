...
public class IndustryDAO {
    ...
    public JSONObject getIndustries() {
        String sql = "CALL sp_getIndustries";
    ...
    public JSONObject getIndustryById(JSONObject json) {
        String sql = "CALL sp_getIndustry(?)";
        try (Connection connect = DriverManager.getConnection("jdbc:mysql://" + server + ":" + port + "/" + database
                + "?user=" + DBusername + "&password=" + DBpassword);
                CallableStatement callableStatement = connect.prepareCall(sql)) {
    ...
    public JSONObject createIndustry(JSONObject json) {
      String sql = "CALL sp_createIndustry(?)";
      try (Connection connect = DriverManager.getConnection("jdbc:mysql://" + server + ":" + port + "/" + database
              + "?user=" + DBusername + "&password=" + DBpassword);
              CallableStatement callableStatement = connect.prepareCall(sql)) {
          callableStatement.setString(1, json.toString());
